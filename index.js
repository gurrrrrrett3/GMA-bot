...
//Trial Command
  ...
  if (command == "trial") {
    ...
    if (list.get(argument) == null) {
      message.channel.send(
        `Sorry, I couldn't fimd a trial with the name: ${argument}. Please refine your search.`
      );
      return;
    }
  ...
  //use the sorting direction flag to sort the scores correctly
    ...
    if (trialData.asc) {
      ...
      scores.sort(
        (a, b) => a.score.replace(",", "") - b.score.replace(",", "")
      );
    }
  ...
  if (command == "wr") {
    ...
    if (message.mentions.members.size > 0) {
      if (
        links.find({ id: message.mentions.members.first().user.id }).value() ==
        undefined
      ) {
        //not a linked account, so return an error
        message.channel.send(
          "Sorry, that user hasn't linked their account yet."
        );
        return;
      } else {
        argument = links
          .find({ id: message.mentions.members.first().user.id })
          .value().name;
      }
    }
    ...
    if (userGuess == null) {
      message.channel.send(
        `Sorry, I couldn't find a user with the name: ${argument}.  Please refine your search.`
      );
      return;
    }
    ...
    if (list.includes(argument)) {
      ...
      if (links.find({ name: argument }).value() == undefined) {
        ...
        if (links.find({ id: message.author.id }).value() != undefined) {
          message.channel.send(
            `Looks like you have already linked a username! Your account is linked to ${
              links.find({ id: message.author.id }).value().name
            }`
          );
          return;
        }
        ...
        message.channel.send(
          `<@${message.member.user.id}>, your account has been linked to your leaderboard username: ${argument}.`
        );
      } else {
        message.channel.send(
          `Sorry, this leaderboard username has already been linked! If this is your username, please DM Gucci Garrett#9211 and he will fix it.`
        );
      }
    } else {
      message.channel.send(
        `Sorry, that username doesn't exist.  Make sure you typed it correctly!`
      );
    }
  }
  ...
  if (command == "notif") {
    if (links.find({ id: message.author.id }).value() == undefined) {
      message.channel.send(
        "Sorry, you need to link your discord to your LB username, as well as have at least 1 score posted to the LB.  Use =link {lb username} to link."
      );
    } else {
      ...
      embed.addField(
        "Get a DM when one of your WR's is broken",
        `🟥: ${toggle(notifData.wrUpdates)}`
      );
      embed.setTimestamp();
      ...
      message.channel.send(embed).then((sentMessage) => {
        ...
        collector.on("collect", (reaction, user) => {
          ...
          newEmbed.setTitle(
            `${message.member.displayName}'s Notification Panel`
          );
          newEmbed.addField(
            "Get a DM when one of your WR's is broken",
            `🟥: ${toggle(notifData.wrUpdates)}`
...
function UpdateDatabase() {
  ...
  function getData(auth) {
            ...
            if (name != undefined) {
              if (
                !(
                  ...
                  name.toLowerCase().includes("triple diamond count") ||
                  name == "Challenge Name"
                ) ||
                name == ""
              ) {
    ...
    userData.map((data) => {
      ...
      if (
        notif.find({ name: data.user.toLowerCase() }).value().wrUpdates == false
      ) {
        return;
      }
    });
    ...
    Client.user.setActivity(
      `on ${trials.value()[Math.floor(Math.random() * trials.value().length)]}`
    );
  }
}
/**
